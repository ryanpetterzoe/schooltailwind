    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl p-6">
        <form method="POST" action="<?= APP_URL ?>/admin/jurusan/<?= $isEdit ? 'edit/'.$program['id'] : 'tambah' ?>" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                        <i class="fas fa-image text-blue-600 mr-1"></i>Gambar Jurusan (Banner Utama)
                    </label>
                ...
                <!-- ============================================================
                     GALERI FOTO KEGIATAN (multi-upload)
                     Tampil di halaman detail jurusan sebagai grid + lightbox
                     ============================================================ -->
                <div class="border-t border-slate-100 dark:border-slate-700 pt-4 mt-2">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                        <i class="fas fa-images text-blue-600 mr-1"></i>Galeri Foto Kegiatan
                    </label>
                    <small class="text-xs text-slate-400 dark:text-slate-500 block mb-3">
                        Upload beberapa foto kegiatan jurusan sekaligus. Tampil sebagai slideshow + grid dengan lightbox di halaman detail. Otomatis dikompres saat upload.
                    </small>
                    ...
                    <div class="mb-4">
                        <div class="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-2">
                            <i class="fas fa-folder-open text-blue-500 mr-1"></i>Foto saat ini (<?= count($programImages) ?>) — seret untuk ubah urutan
                        </div>
                        <div id="existingGallery" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                            <?php foreach ($programImages as $idx => $pi): ?>
                            <div class="existing-image-card relative group bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden cursor-move"
                                 draggable="true"
                                 data-image-id="<?= (int)$pi['id'] ?>">
                                <input type="hidden" name="existing_image_id[]" value="<?= (int)$pi['id'] ?>">
                                <input type="hidden" name="existing_image_sort[]" class="sort-input" value="<?= (int)$pi['sort_order'] ?>">
                                <div class="aspect-video overflow-hidden bg-slate-200 dark:bg-slate-700">
                                    <img src="<?= UPLOAD_URL . htmlspecialchars($pi['image']) ?>" alt="<?= htmlspecialchars($pi['caption'] ?? '') ?>" class="w-full h-full object-cover">
                                </div>
                                <div class="p-2">
                                    <input type="text"
                                           name="existing_image_caption[]"
                                           value="<?= htmlspecialchars($pi['caption'] ?? '') ?>"
...
<!-- ============================================================
     GALLERY MANAGEMENT (multi-upload, delete, drag-reorder)
     ============================================================ -->
<script>
(function () {
    ...
    if (gallery) {
        gallery.addEventListener('click', function (e) {
            ...
            // Remove the card from the DOM (its hidden inputs go with it,
            // so the server won't try to update a deleted row)
            card.remove();
            renumberOrderBadges();
        });
    }
    ...
    /* ---- Preview & caption fields for new uploads ---- */
    ...
    /* ---- Existing single banner preview ---- */
    var bannerInput = document.querySelector('input[name="image"]');
    var bannerPreview = document.getElementById('progImgPreview');
