<?php
$pageTitle = htmlspecialchars($ekskul['name'] ?? 'Detail Ekstrakurikuler') . ' - ' . ($settings['school_name'] ?? 'Sekolah');
$ekskulImages = $ekskulImages ?? [];
$hasGallery = !empty($ekskulImages);
$hasBanner  = !empty($ekskul['image']);
...
if ($hasGallery) {
    ...
    $slides[] = ['image' => $ekskul['image'], 'caption' => ''];
}
...
<section class="relative bg-gradient-to-br from-blue-600 to-indigo-700 py-16 overflow-hidden">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
    <h1 class="text-3xl sm:text-4xl font-extrabold text-white mb-2"><?= htmlspecialchars($ekskul['name'] ?? '') ?></h1>
    <nav class="flex items-center gap-2 text-sm">
      ...
      <a href="<?= APP_URL ?>/ekskul" class="text-white/70 hover:text-white transition-colors">Ekstrakurikuler</a>
      <i class="fas fa-chevron-right text-white/40 text-xs"></i>
      <span class="text-white"><?= htmlspecialchars($ekskul['name'] ?? '') ?></span>
    </nav>
  </div>
</section>
    ...
    <div class="grid lg:grid-cols-3 gap-8">
      <div class="lg:col-span-2">
        ...
        <!-- ============================================================
             ...
             ============================================================ -->
        <?php if (!empty($slides)): ?>
        <div id="ekskulCover" class="relative rounded-2xl overflow-hidden mb-6 shadow-lg bg-slate-200 dark:bg-slate-800 aspect-video">
          <?php foreach ($slides as $i => $s): ?>
          <div class="ekskul-slide absolute inset-0 transition-opacity duration-700 ease-in-out <?= $i === 0 ? 'opacity-100' : 'opacity-0 pointer-events-none' ?>"
               data-index="<?= $i ?>">
            <img src="<?= UPLOAD_URL . htmlspecialchars($s['image']) ?>"
                 alt="<?= htmlspecialchars($s['caption'] ?: $ekskul['name']) ?>"
          ...
          <div class="absolute bottom-4 left-4 flex items-center gap-3 pointer-events-none">
            <div class="w-12 h-12 sm:w-14 sm:h-14 bg-white/95 dark:bg-slate-800 rounded-xl flex items-center justify-center text-blue-600 text-lg sm:text-xl shadow-lg">
              <i class="<?= htmlspecialchars($ekskul['icon'] ?? 'fas fa-futbol') ?>"></i>
            </div>
            <div class="text-white">
              <div class="text-[10px] sm:text-xs uppercase tracking-wider opacity-80">Ekstrakurikuler</div>
              <div class="font-bold text-sm sm:text-lg drop-shadow"><?= htmlspecialchars($ekskul['name'] ?? '') ?></div>
            </div>
          </div>
          ...
          <div class="ekskul-slide-caption absolute bottom-4 right-4 max-w-[60%] px-3 py-1.5 bg-slate-900/70 backdrop-blur-sm text-white text-xs sm:text-sm rounded-lg <?= $i === 0 ? '' : 'hidden' ?>"
               data-caption-for="<?= $i ?>">
          ...
          <button type="button" id="ekskulPrev" aria-label="Sebelumnya"
          ...
          <button type="button" id="ekskulNext" aria-label="Berikutnya"
          ...
          <div class="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
            <?php foreach ($slides as $i => $s): ?>
            <button type="button"
                    data-ekskul-indicator="<?= $i ?>"
                    class="ekskul-indicator h-1.5 rounded-full transition-all duration-300 <?= $i === 0 ? 'w-6 bg-white' : 'w-1.5 bg-white/50' ?>"></button>
            <?php endforeach; ?>
          </div>
          <?php endif; ?>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-6 sm:p-8">
          ...
          <div class="flex items-center gap-4 mb-6">
            ...
            <div>
              <h2 class="text-xl font-bold text-slate-800 dark:text-white"><?= htmlspecialchars($ekskul['name'] ?? '') ?></h2>
            </div>
          </div>
          <?php endif; ?>
          <div class="text-slate-600 dark:text-slate-300 leading-relaxed prose prose-slate dark:prose-invert max-w-none"><?= safeRichHtml($ekskul['description'] ?? '') ?></div>
        </div>
        ...
        <div class="mt-8">
          <div class="flex items-center gap-2 mb-4">
            ...
            <div>
              <h5 class="font-bold text-slate-800 dark:text-white">Foto Kegiatan Ekskul</h5>
              <p class="text-xs text-slate-400"><?= count($ekskulImages) ?> foto · klik untuk perbesar</p>
            </div>
          </div>
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            <?php foreach ($ekskulImages as $ei): ?>
            <button type="button"
                    ...
                    data-lightbox-src="<?= UPLOAD_URL . htmlspecialchars($ei['image']) ?>"
                    data-lightbox-caption="<?= htmlspecialchars($ei['caption'] ?? '') ?>">
              <img src="<?= UPLOAD_URL . htmlspecialchars($ei['image']) ?>"
                   alt="<?= htmlspecialchars($ei['caption'] ?? $ekskul['name']) ?>"
        ...
        <div class="mt-8">
          <div class="flex items-center justify-between mb-4">
            <h5 class="font-bold text-slate-800 dark:text-white flex items-center gap-2"><i class="fas fa-newspaper text-blue-500"></i>Berita Ekskul Ini</h5>
          </div>
      ...
      <div class="space-y-6">
        <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-6">
          <h5 class="font-bold text-slate-800 dark:text-white mb-4">Informasi Ekskul</h5>
          <div class="border-t border-slate-100 dark:border-slate-700 pt-4 space-y-3">
            <div class="flex justify-between"><span class="text-sm text-slate-400">Nama</span><strong class="text-sm text-slate-700 dark:text-white"><?= htmlspecialchars($ekskul['name'] ?? '-') ?></strong></div>
            <?php if (!empty($ekskulImages)): ?>
            <div class="flex justify-between"><span class="text-sm text-slate-400">Foto Kegiatan</span><strong class="text-sm text-slate-700 dark:text-white"><?= count($ekskulImages) ?> foto</strong></div>
            <?php endif; ?>
          </div>
        </div>
        <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-6 text-center">
          <h5 class="font-bold text-slate-800 dark:text-white mb-2">Tertarik Bergabung?</h5>
          <p class="text-sm text-slate-400 mb-4">Hubungi kami untuk informasi lebih lanjut tentang ekskul ini</p>
          <a href="<?= APP_URL ?>/kontak" class="block w-full py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-semibold text-sm shadow-lg shadow-blue-500/25 hover:-translate-y-0.5 transition-all">Hubungi Kami</a>
        </div>
      </div>
    </div>
  </div>
...
<div id="ekskulLightbox"
  ...
  <img id="ekskulLightboxImg" src="" alt="" class="max-w-full max-h-[85vh] rounded-lg shadow-2xl">
  <div id="ekskulLightboxCaption" class="absolute bottom-6 left-1/2 -translate-x-1/2 max-w-[90%] px-4 py-2 bg-slate-900/80 text-white text-sm rounded-lg hidden"></div>
</div>

<script>
(function () {
    /* ---- Slideshow ---- */
    var cover  = document.getElementById('ekskulCover');
    if (cover) {
        var slides     = cover.querySelectorAll('.ekskul-slide');
        var captions   = cover.querySelectorAll('.ekskul-slide-caption');
        var indicators = cover.querySelectorAll('.ekskul-indicator');
        var prevBtn    = document.getElementById('ekskulPrev');
        var nextBtn    = document.getElementById('ekskulNext');
        var current    = 0;
        var auto       = null;
        var paused     = false;
        ...
        if (slides.length > 1) {
            ...
            indicators.forEach(function (b) {
                b.addEventListener('click', function () {
                    show(parseInt(b.getAttribute('data-ekskul-indicator'), 10));
                    startAuto();
                });
            });
    ...
    /* ---- Lightbox ---- */
    var lb        = document.getElementById('ekskulLightbox');
    var lbImg     = document.getElementById('ekskulLightboxImg');
    var lbCap     = document.getElementById('ekskulLightboxCaption');
    var lbClose   = document.getElementById('ekskulLightboxClose');
